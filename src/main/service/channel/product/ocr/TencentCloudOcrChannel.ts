...
class TencentCloudOcrChannel implements IOcrInterface {
  /**
   * OCR
   *
   * @param info OCR信息
  ...
  async apiOcrCheck(info): Promise<void> {
    TencentCloudRequest.apiOcr(info).then(
      ...
      (err) => {
        log.error('[腾讯云OCR校验密钥事件] - 异常响应报文 : ', err)
        let msg = ''
        const errMessage = err.message
...
export default TencentCloudOcrChannel
